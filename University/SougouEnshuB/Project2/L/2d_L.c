// Code by Yuto Kizawa 2022/11/14
...
int main(void)
{
    ...
    fp1 = fopen("2d_temperature_for_gif_L.dat", "w");
    ...
    // 追加部分 境界条件が 0 なら不要
    /*
    theta_current[N/2 + 1][N/2 + 1] = 0.0;
    theta_next[N/2 + 1][N/2 + 1] = 0.0;
    for(i = N/2 + 2; i < N; i++)
    {
        theta_current[i][N/2 + 1] = 0.0;
        theta_next[i][N/2 + 1] = 0.0;

        theta_current[N/2 + 1][i] = 0.0;
        theta_next[N/2 + 1][i] = 0.0;
    }
    */
    ...
    theta_next[0][N] = 0.0;
    theta_current[N][0] = 0.0;
    theta_next[N][0] = 0.0;
    theta_current[N][N] = 0.0;
    theta_next[N][N] = 0.0;
    */
