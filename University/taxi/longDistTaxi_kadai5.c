...
typedef struct searchNode {
  ...
  int drivingRange;  // 課題5で追加 走行可能距離
 ...
 */
priorityQ_t Q;
int cost[MAX_N_CITIES][2];  // 課題5で変更
...
int solve(int n, int m, int cap) {
  ...
  while(qSize(&Q) > 0) {
    ...
    for(j=0; j<roadinfo[here.cityID].num; j++) { /* 隣接ノードをenqueue　1ループが1隣接ノードに対する操作にあたる*/ 
      ...
      int nextDrivingRange = here.drivingRange - distance;  // 課題5で追加
      // int nextDrivingRange = (withGas[here.cityID] == false) ? (here.drivingRange - distance) : cap * 10;  // 課題5で追加
      
      /*
      if(withGas[here.cityID] == false)
      {
        searchNode_t next = {nextPathLen, destination, nextDrivingRange};
      }
      else
      {
        searchNode_t next = {nextPathLen, destination, cap*10};
      }
      */
      
      /* 
      if((cost[destination] != INT_MAX && cost[destination] < nextPathLen) || nextDrivingRange < 0)  // ここに問題がありそう
      {
        continue;
      }
      else
